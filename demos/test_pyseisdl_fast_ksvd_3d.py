...
## benchmark with fast KSVD
t1 = time.time()
param={'T':2,'niter':10,'mode':1,'K':64};
[d3,D3,G3,dct3]=dl.fast_ksvd_denoise(dn,mode,[l1,l2,l3],[s1,s2,s3],perc,param);
t2 = time.time()
print('fast_KSVD takes %.2g seconds'%(t2-t1));
...
datas = {"dc":dc, "dn":dn, "d1": d1, "d2": d2, "d3":d3}
...
print('SNR of fast KSVD is %g'%dl.snr(dc,d3,2));
...
plt.imshow(d3.reshape(n1,n2*n3,order='F'),cmap='jet',clim=(-0.1, 0.1),aspect='auto');ax.set_xticks([]);ax.set_yticks([]);
plt.title('Denoised (fast_KSVD, SNR=%.4g dB)'%dl.snr(dc,d3,2));
ax=fig.add_subplot(4, 2, 8)
plt.imshow(noi3.transpose(0,2,1).reshape(n1,n2*n3),cmap='jet',clim=(-0.1, 0.1),aspect='auto');ax.set_xticks([]);ax.set_yticks([]);
plt.title('Noise (fast_KSVD)');
