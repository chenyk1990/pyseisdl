def sgk(X,param):
	...
	# size of G: KxN
	#
	# DEMO: demos/test_sgk_denoise.py
...
def omp_e( D, x ):
	...
	g[k]=np.sum(D[:,k]*x)/np.sum(D[:,k]*D[:,k]); 	#option 1: more accurate sparse coding (according to definition of sparse coding)
...
def omp0( D, x, K ):
	...
	for ik in range(0,K):
    	...
    	for i2=1:n2                 	#loop over all atoms (greedy algorithm)       
        	if np.sum(np.where(I==i2)[0])==0:  	#search among the other atoms
